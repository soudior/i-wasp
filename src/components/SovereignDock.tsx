...
import { Home, User, Flag, ShoppingBag, Crown } from 'lucide-react';
        ...
        <div className="flex items-center gap-2 px-4 py-3 rounded-[2rem] bg-black/80 backdrop-blur-2xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
          ...
          {!isGold && (
            <motion.button
              ...
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
              className="flex flex-col items-center gap-1 px-5 py-2 rounded-2xl ml-2 relative overflow-hidden"
              style={{ 
      ...
      {showUpgrade && (
        <SubscriptionUpgrade 
          ...
          onSuccess={() => {
            ...
            // Refresh the page to apply subscription changes
